...
**Important:** Make sure that the MagicStick device is first **reset to factory settings**, so that it holds no previous paired keyboard data in its internal memory, by following [these steps](#reset).
...
as well as to change the keyboard's special keys configuration. You can also use it for permanently turning the MagicStick Bluetooth chip on or off, for instance if you are in a very high IT security work environment.
...
![](20230928224646.png)
which readily recognises MagicStick and shows the connected keyboard's battery level.
...
| **1 flash** and a pause. | **IDLE**. MagicStick is operational but no keyboard is connected via wire connection in or by Bluetooth. |
| **2 flashes** and a pause. | **Bluetooth** **CONNECTING**. MagicStick Bluetooth is trying to connect to an already paired device via Bluetooth. |
...
**Second way:** Unplug MagicStick and then plug it in with the bootsel button being constantly pressed. This will enter the MagicStick into bootsel mode.
